	...
	std::thread thr([&] {
		...
		while (!isExit)
		{
			...
			for (auto iter = lis.begin(); iter != lis.end(); ++iter)
			{
				...
				ofs << (std::chrono::duration_cast<std::chrono::hours>(now_log)).count()<<":"
					<< (std::chrono::duration_cast<std::chrono::minutes>(now_log)).count() << ":"
					<< (std::chrono::duration_cast<std::chrono::seconds>(now_log)).count() << "."
					<< (std::chrono::duration_cast<std::chrono::milliseconds>(now_log)).count() << "'"
					<< (std::chrono::duration_cast<std::chrono::microseconds>(now_log)).count() << "''"
					<< (std::chrono::duration_cast<std::chrono::nanoseconds>(now_log)).count() 
					<< ",";
